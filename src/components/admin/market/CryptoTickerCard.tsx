...
import { marketApi, fmtPrice, type PriceRow } from '../../../services/market';

const SYMBOLS = ['BTC', 'ETH', 'SOL', 'SUI', 'XRP'];

interface Props {
  /** Cotação USD→BRL (do FinancialView) para exibir o equivalente em reais. */
  usdBrl?: number;
}

// Card de cotações de cripto ao vivo para o painel Financeiro.
const CryptoTickerCard: React.FC<Props> = ({ usdBrl }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setRows(await marketApi.prices(SYMBOLS));
          ...
          {SYMBOLS.map(sym => {
            ...
            return (
              <div key={sym} style={{ background: 'var(--color-surface-2)', border: '1px solid var(--color-border)', borderRadius: 10, padding: '12px 14px' }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>{r ? fmtPrice(r.price) : '—'}</div>
                <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 2 }}>
                  {r && usdBrl ? `≈ ${fmtPrice(r.price * usdBrl, 'BRL')}` : (r ? '' : 'carregando…')}
                </div>
              </div>
            );
          })}
